function model = multivariate_spectrum_model(data, fs, N_spect, M_set, f, verbose, criterion)
%% spectrum_model
%
...
count_max = 20;                     % Maximum number of retries when the 
...
% energy spectrum
aux = fft(data)';
aux = aux(1:length(aux)/2+1);
aux = [real(aux);imag(aux)];
spectrum_norm = N_spect * aux(:,1:N_spect) ./ repmat(sum( aux(:,1:N_spect),2 ), [1,N_spect]);
...
for m = 1:length(M_set)
    ...
    while (~ok) && (count < count_max)
        ...
        while ~stop
            ...
            mycdf = repmat(diff(mixture_normcdf(f', mu, sqrt(sigma), p_w)),[M,1]);
            A_1 = rp_w .* G_1 ./ mycdf;
            A_2 = rp_w .* G_2 ./ mycdf;
            ...
            A_3 = repmat(p_old,[1,N_spect]) .* G_3 ./ repmat(diff(mixture_normcdf(f', mu, sqrt(sigma), p_w)),[M,1]);
            sigma = sum(rspectrum_norm .* A_3,2) ./ evidence;
            
            % Stopping criterion
            iter = iter + 1;
            L(iter) = sum(spectrum_norm .* log(diff(mixture_normcdf(f', mu, sqrt(sigma), p_w))));
            if isnan(L(iter))
                stop = 1;
                ok = 0;
                if verbose
